...
class IsPreAuthToken(BasePermission):
    ...
    This can be used to restrict access to certain views until
    the user has completed pre-authentication steps.
    """
    ...
    def has_permission(self, request, view):
        ...
        return token is not None and \
            getattr(token, 'token_type', None) == 'pre_auth'
...
class IsFullAuthToken(BasePermission):
    ...
    until the user has completed full authentication steps.
    """
    ...
    def has_permission(self, request, view):
        ...
        token = getattr(request, 'auth', None)
        return token is not None and \
            getattr(token, 'token_type', None) == 'full_auth'
